import { validateMnemonic, mnemonicToEntropy, generateMnemonic, mnemonicToSeed } from 'bip39';
import { fromBase58, fromSeed } from 'bip32';
import { createHash, randomBytes, pbkdf2Sync, createCipheriv, createHmac } from 'crypto-browserify';
...
function () {
  ...
  _proto.getAppNode = function getAppNode(appDomain) {
    var hash = createHash('sha256').update("" + appDomain + this.salt).digest('hex');
...
function getIdentityOwnerAddressNode(identityPrivateKeychain, identityIndex) {
  ...
  var salt = createHash('sha256').update(publicKeyHex).digest('hex');
...
function getAddress(node) {
  return address.toBase58Check(crypto.hash160(node.publicKey), networks.bitcoin.pubKeyHash);
}
...
var encryptMnemonic = function encryptMnemonic(mnemonic, password) {
  try {
    ...
    return Promise.resolve(normalizeMnemonic(mnemonic)).then(function (normalizedMnemonic) {
      ...
      var keysAndIV = pbkdf2Sync(password, salt, 100000, 48, 'sha512');
      var encKey = keysAndIV.slice(0, 16);
      var macKey = keysAndIV.slice(16, 32);
      var iv = keysAndIV.slice(32, 48);
      var cipher = createCipheriv('aes-128-cbc', encKey, iv);
      ...
      var hmac = createHmac('sha256', macKey);
...
var normalizeMnemonic = function normalizeMnemonic(mnemonic) {
  try {
    ...
    return Promise.resolve(mnemonicToEntropy(mnemonic));
  } catch (e) {
    return Promise.reject(e);
  }
};
...
var makeGaiaAssociationToken = function makeGaiaAssociationToken(secretKeyHex, childPublicKeyHex) {
  ...
  var compressedPublicKeyHex = getPublicKeyFromPrivate(signerKeyHex);
...
function () {
  ...
  _proto.makeAuthResponse = function makeAuthResponse$1(_ref2) {
    ...
    try {
      ...
      return Promise.resolve(getHubPrefix(gaiaUrl)).then(function (hubPrefix) {
        return Promise.resolve(_this2.profileUrl(hubPrefix)).then(function (profileUrl) {
          ...
          var compressedAppPublicKey = getPublicKeyFromPrivate(appPrivateKey.slice(0, 64));
          var associationToken = makeGaiaAssociationToken(_this2.keyPair.key, compressedAppPublicKey);
          return makeAuthResponse(_this2.keyPair.key, profile || {}, '', {
  ...
  _proto.appPrivateKey = function appPrivateKey(appDomain) {
    ...
    var appsNode = new AppsNode(fromBase58(appsNodeKey), salt);
...
function () {
  ...
  Wallet.generate = function generate(password) {
    try {
      ...
      var backupPhrase = generateMnemonic(STRENGTH, randomBytes);
      return Promise.resolve(mnemonicToSeed(backupPhrase)).then(function (seedBuffer) {
        var masterKeychain = fromSeed(seedBuffer);
